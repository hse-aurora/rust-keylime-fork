...
use tss_esapi::{
    ...
    attributes::{
        session::SessionAttributesBuilder, object::ObjectAttributesBuilder,
    },
    constants::{
    ...
    interface_types::{
        algorithm::{HashingAlgorithm, PublicAlgorithm, AsymmetricAlgorithm,}, 
    ...
    structures::{
        ...
        SignatureScheme, PublicBuilder, EccParameter, EccPoint, EccScheme, 
        KeyDerivationFunctionScheme, PublicEccParametersBuilder, PublicKeyRsa, 
        PublicRsaParametersBuilder, RsaExponent, RsaScheme, 
        SymmetricDefinitionObject, HashScheme, Data,
    },
...
impl Context {
     ...
     pub fn create_idevid(
        ...
        let pcr_selection_list = PcrSelectionListBuilder::new()
            .with_selection(HashingAlgorithm::Sha256,
                            ...
                            PcrSlot::Slot2,PcrSlot::Slot3,
                            PcrSlot::Slot4,PcrSlot::Slot5,
                            PcrSlot::Slot6,PcrSlot::Slot7,
                            ])
            .build()
            .expect("Failed to build PcrSelectionList");
                
        let primary_key = self.inner.execute_with_nullauth_session(|ctx| {
            ctx.create_primary(
    ...
    pub(crate) fn create_idevid_public_from_default_template(
        ...
        let (auth_policy, key_bits, curve_id) = match name_alg {
            HashingAlgorithm::Sha256 => (IDEVID_AUTH_POLICY_SHA256[0..32].to_vec(), RsaKeyBits::Rsa2048, EccCurve::NistP256),
            HashingAlgorithm::Sm3_256 => (IDEVID_AUTH_POLICY_SHA256[0..32].to_vec(), RsaKeyBits::Rsa2048, EccCurve::Sm2P256),
            HashingAlgorithm::Sha384 => (IDEVID_AUTH_POLICY_SHA384[0..48].to_vec(), RsaKeyBits::Rsa3072, EccCurve::NistP384),
            HashingAlgorithm::Sha512 => (IDEVID_AUTH_POLICY_SHA512[0..64].to_vec(), RsaKeyBits::Rsa4096, EccCurve::NistP521),
            _ => (IDEVID_AUTH_POLICY_SHA256[0..32].to_vec(), RsaKeyBits::Rsa2048, EccCurve::NistP256),
        };
        ...
        let key_builder = match asym_alg {
            AsymmetricAlgorithm::Rsa => PublicBuilder::new()
                ...
                .with_rsa_unique_identifier(PublicKeyRsa::try_from(&UNIQUE_IDEVID[0..6])?),
            ...
            .with_object_attributes(obj_attrs)
            .with_auth_policy(Digest::try_from(IDEVID_AUTH_POLICY_SHA256[0..32].to_vec())?)
            .with_rsa_parameters(
                PublicRsaParametersBuilder::new()
                .with_symmetric(SymmetricDefinitionObject::Null)
                .with_scheme(RsaScheme::Null)
    ...
    pub(crate) fn create_iak_public_from_default_template(
        ...
        let (auth_policy, key_bits, curve_id) = match name_alg {
            HashingAlgorithm::Sha256 => (IAK_AUTH_POLICY_SHA256[0..32].to_vec(), RsaKeyBits::Rsa2048, EccCurve::NistP256),
            HashingAlgorithm::Sm3_256 => (IAK_AUTH_POLICY_SHA256[0..32].to_vec(), RsaKeyBits::Rsa2048, EccCurve::Sm2P256),
            HashingAlgorithm::Sha384 => (IAK_AUTH_POLICY_SHA384[0..48].to_vec(), RsaKeyBits::Rsa3072, EccCurve::NistP384),
            HashingAlgorithm::Sha512 => (IAK_AUTH_POLICY_SHA512[0..64].to_vec(), RsaKeyBits::Rsa2048, EccCurve::NistP521),
            _ => (IAK_AUTH_POLICY_SHA256[0..32].to_vec(), RsaKeyBits::Rsa2048, EccCurve::NistP256),
        };
        ...
        let key_builder = match asym_alg {
            AsymmetricAlgorithm::Rsa => PublicBuilder::new()
                ...
                .with_rsa_unique_identifier(PublicKeyRsa::try_from(&UNIQUE_IAK[0..3])?),
            AsymmetricAlgorithm::Ecc => PublicBuilder::new()
                .with_public_algorithm(PublicAlgorithm::Ecc)
                .with_name_hashing_algorithm(name_alg)
                .with_object_attributes(obj_attrs)
            ...
            .with_name_hashing_algorithm(HashingAlgorithm::Sha256)
            .with_object_attributes(obj_attrs)
            .with_auth_policy(Digest::try_from(IAK_AUTH_POLICY_SHA256[0..32].to_vec())?)
            .with_rsa_parameters(
                PublicRsaParametersBuilder::new()
                .with_symmetric(SymmetricDefinitionObject::Null)
                .with_scheme(RsaScheme::Null)
    ...
    pub fn certify_credential_with_iak(
        ...
        self.inner
            .execute_with_sessions(
                ...
                |context| {
                    context.certify(ak.into(), iak, qualifying_data, SignatureScheme::Null)
                },
            )
            .map_err(TpmError::from)
    }
